...
export default function About() {
  ...
  const stats = [
    {
      icon: 'schedule',
      value: settings.stat_experience_years || '3+',
      label: 'Years of Experience',
    },
    {
      icon: 'rocket_launch',
      value: settings.stat_projects_completed || '25+',
      label: 'Projects Done',
    },
    {
      icon: 'handshake',
      value: settings.stat_client_satisfaction || '99%',
      label: 'Happy Clients',
    },
  ]

  const aboutTitle = settings.about_title || 'About Me'
  const desc1 = settings.about_description_1 || "I'm Adji Bhawantu — a developer who doesn't just write code, but builds products that are actually used. Every project starts with one question: what does the client truly need? The result isn't just a website — it's a digital solution that works."
  const desc2 = settings.about_description_2 || 'With experience across diverse project types, from business systems to modern web platforms, I ensure every product is not only functional but also intuitive and enjoyable for its users.'
    ...
    <section ref={sectionRef} className="px-gutter py-section-padding-y max-w-container-max mx-auto" id="about">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 reveal">
        <div className="md:col-span-7 card-bento p-8 md:p-12 flex flex-col justify-center card-hover card-hover-102">
          <h2 className="font-headline-md text-headline-md text-on-surface mb-4">{aboutTitle}</h2>
